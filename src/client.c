...
int ftp_ls(struct ftp_connection *conn, struct list *list, char *path) {
    ...
    //create the command
    snprintf(commandbuf, sizeof(commandbuf) - 1, "LIST %s\r\n", path);

    //send the command and receive the response
    send(conn->control_conn, commandbuf, strlen(commandbuf), 0);
    if(read(conn->control_conn, response, sizeof(response) - 1) < 0) {
        fprintf(stderr, "Could not read LIST command response.\n");
        return -1;
    }
    ...
    //keep a running buffer of the current line
    char current_line[1024];
    char *lineptr = current_line;
    int isnewline = 0;

    //initialize the current line and response to all 0s
    memset(current_line, 0, sizeof(current_line));
    memset(response, 0, sizeof(response));

    //read in all the data from the data connection
    while(read(conn->data_conn, response, sizeof(response) - 1) > 0) {
        ...
        while(*responseptr != 0) {
            *lineptr++ = *responseptr++;
            ...
            //newline
            if(*responseptr == '\r') {
                //information pulled out of a line
                isnewline = 1;
                ...
                join(filepath, path, filename, sizeof(filepath) - 1);

                //add the file to the list
                append(list, filepath);
                
                //reset the line buffer
                memset(current_line, 0, sizeof(current_line));
                lineptr = current_line;
                responseptr++;
